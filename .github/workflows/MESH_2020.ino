...
//Reading analog values of first and last sensors
  qtr.setTypeAnalog();
  qtr.setSensorPins((const uint8_t[]){A0,A7}, SensorCount);
...
void loop(){
    ...
    if(IR_TOP == 1)
    {
         if(IR_LEFT==1 && IR_RIGHT==1) {             //90 deg cross
            encoder_stop=myEnc.read();
            assign_temp_point();
            if(check()){
                points[len].x_cordinate=temp_x;
        ...
        else if(IR_LEFT==0 && IR_RIGHT==1){           //right and straight
            encoder_stop=myEnc.read();
            assign_temp_point();
            if(check()){
                points[len].x_cordinate=temp_x;
